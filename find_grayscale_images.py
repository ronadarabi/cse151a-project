...
def is_grayscale_fast(image_path, tolerance=0.1):
    try:
        with Image.open(image_path) as img:
            ...
            samples = img_array[np.random.randint(0, h, sample_size), np.random.randint(0, w, sample_size)]

            if np.max(np.std(samples, axis=1)) < tolerance:
                return image_path
...
def find_grayscale_images(root_dir, tolerance=0.1, chunk_size=1000):
    all_images = []
...
if __name__ == "__main__":
    ...
    grayscale_images = find_grayscale_images(root_dir, tolerance, chunk_size)
